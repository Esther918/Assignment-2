...
class BeamElement3D:
    def __init__(self, id, node_i_id, node_j_id, E, nu, A, Iy, Iz, J, nodes, v_temp=None):
        self.id = id
        self.node1 = nodes[node_i_id]
        self.node2 = nodes[node_j_id]
        self.E = E
        self.nu = nu
        self.A = A
        self.Iy = Iy
        ...
        # Compute element length and local coordinate system
        x1, y1, z1 = self.node1.x, self.node1.y, self.node1.z
        x2, y2, z2 = self.node2.x, self.node2.y, self.node2.z
        self.L = np.sqrt((x2 - x1)**2 + (y2 - y1)**2 + (z2 - z1)**2)

        # Compute local z-axis (using rotation_matrix_3D)
        self.T = transformation_matrix_3D(rotation_matrix_3D(x1, y1, z1, x2, y2, z2, v_temp))
        self.local_z = self.T[:3, 2]  # Extract local z-axis from transformation matrix
